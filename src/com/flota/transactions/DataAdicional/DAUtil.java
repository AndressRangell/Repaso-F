...
public class DAUtil {
    ...
    public static final String TAG = "DAUtil";

    private String msgId;
    private List<SubFieldsModel> subFieldsModel = new ArrayList<>();
    private ArrayList<SubField> decoFields = new ArrayList<>();
    private int index = 0;

    /**
     * @param msgId -> Identificador del tipo de mensaje
     */
    public DAUtil(String msgId) {
        this.msgId = msgId;
        loadSubFieldsModel();
    }
     ...
     */
    public List<SubFieldsModel> getSubTrans() {
     ...
     */
    private void loadSubFieldsModel() {
        ...
        subFieldsModel.add(new SubFieldsModel("0200", "830000", new int[]{8, 10, 33, 70, 81, 90, 94, 95}));
    }
     ...
     */
    private ArrayList<SubField> getSubFieldsList() {
        return decoFields;
    }
    ...
    public void setSubCampos(String field63) {
        ...
        for (int i = 0; i < field63.length(); i = index) {
            String fieldLen = field63.substring(index, index += 4);
            String fieldId = ISOUtil.hex2AsciiStr(field63.substring(index, index + 4));
            String fieldData = obtenerCampo(field63);
            decoFields.add(new SubField(fieldId, fieldData));
            index += Integer.parseInt(fieldLen) * 2;
        }
    }
    ...
    private String obtenerCampo(String campo63) {
        if (!campo63.equals("")) {
            try {
                int lenCampo = Integer.parseInt(campo63.substring(index - 4, index)) * 2;
                int lenReal = (index) + lenCampo;
                if (campo63.length() < ((index) + lenCampo)) {
                    lenReal = campo63.length();
                }
                String data = campo63.substring(index + 4, lenReal);
                return ISOUtil.hex2AsciiStr(data);
            } catch (NumberFormatException ex) {
                Logger.exception(TAG, "Error NumberFormatException");
                return null;
            }
        } else {
            return null;
    ...
    private String getField63(int[] subPrint) {
        ...
        if (subPrint.length > 0) {
            ...
            for (SubField field : getSubFieldsList()) {
                ...
                Logger.error(TAG, "getField63: exists " + exist);
                if (exist) {
            ...
            Logger.error(TAG, "getField63: Array Vacio");
    ...
    private void setCampo90() {
        ...
        try {
            ...
            Logger.error(TAG, "setCampo90: verAux " + verAxu);
